 ...
 *
 * Task of this function is to leave no empty spaces in viewpor.
 *
...
export function shuffleSlides(
    ...
    // Let's mark them now:
    if (unmarkedItemIndexes.length) {
        const occurenceMap = collectOccurences(
            slides,
            offset,
            slideWidth,
            viewportWidth,
        ...
        const unmarkedItemIndexSet = Array.from(new Set(unmarkedItemIndexes));
        for (const itemIndex of unmarkedItemIndexSet) {
            const occurence = occurenceMap.get(itemIndex);
            if (occurence) {
                // There should not be any removable slides,
                // but let's mark them anyway...
                const occurenceIndexes = occurence.removableSlideIndexes
                    .concat(occurence.unremovableSlideIndexes);
                for (const slideIndex of occurenceIndexes) {
                    slides[slideIndex].options.isCopy = true;
                }
            }
        }
    }
...
export function moveOrCopySlidesToEnd(
    ...
    for (let i = 0; i < quantity; i++) {
        ...
        if (shouldCopy || firstSlideHasNextItemId) {
            ...
            // Once inViewport slide was reached, all other slides are
            // considered copies
            if (!canIncreaseQuantity) {
                currentSlide.options.isCopy = true;
                newSlide.options.isCopy = true;
            }
            ...
            const newSlide = new CarouselSlide(
                ...
                nextItemId,
                {
                    // Every filled item is considered is copy, because
                    // we assume that carousel is already filled with
                    // every kind of item
                    isCopy: true,
                    // Every filled item is considered to be in viewport,
                    // because why else would we call this function
